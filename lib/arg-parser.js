...
function ArgParser() {
    ...
    /**
     * Add a new parse rule.  During parsing, each argument which this rule
     * matches will be passed to the parse function.
    ...
    /**
     * Define a default value for an option which has not been set.  This method
     * is chainable.
     * @param {string} opt
     * @param {*} val
     * @returns {ArgParser}
     */
    this.default = function(opt, val) {
        defaults[opt] = val;
        return this;
    };
...
/**
 * Define a basic option which requires an argument value to follow.  The
 * corresponding result name will be set to the the value.  Any number of
...
ArgParser.prototype.option = function(name) {
    ...
    if (!parser.paramOptions) prop.readonly(parser, "paramOptions", []);
    
    names.forEach(function(name) {
        parser.paramOptions.push(name);
...
/**
 * Define a flag option.  When this option is present, the corresponding result
 * name will be true.  Any number of additional names can ba provided to be
 * used as aliases.  This method is chainable.
 * @param {...string} name
 * @returns {ArgParser}
 */
...
/**
 * Define a count option.  The corresponding result name will be set to the
 * number of times the option is passed.  Any number of additional names can be
...
/**
 * Define a list option which requires a following argument and can be
 * specified multiple times.  The corresponding result name will be set to an
 * array of the argument values.  This method is chainable.
 * @param {...string} name
 * @returns {ArgParser}
 */
...
ArgParser.prototype.shortOpts = function() {
    ...
    return this.rule(/^-[^-]+/, function(result, args, arg) {
        var paramOptions = parser.paramOptions || [];
        ...
        // break up combined short opts
            ...
            prefix = paramOptions.indexOf(arg.substr(0,2)) >= 0 ? "" : "-";
...
ArgParser.prototype.longOpts = function(delims) {
    ...
    return this.rule(/^--.+/, function(result, args, arg) {
        var opt, val, i,
            paramOptions = parser.paramOptions || [];
        ...
        for (i = 3; i < arg.length; i++) {
            if (delims.indexOf(arg[i]) >= 0) {
                ...
                if (parser.exactMatch(opt) && paramOptions.indexOf(opt) === -1)
...
/**
 * Add an option which will stop option parsing; all remaining arguments
